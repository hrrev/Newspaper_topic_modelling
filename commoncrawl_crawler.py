...
def crawl_from_commoncrawl(callback_on_article_extracted, valid_hosts=None, start_date=None, end_date=None,
    ...
    all_links = __get_remote_index()
    
    relevant_links = []
    for link in all_links:
        #splicing the string of date from url name
        link_date_str = link[35:49]
        link_date = parser.parse(link_date_str)
        #checking if we should take the link or not
        if link_date >= start_date and link_date <= end_date:
            relevant_links.append(link)
            
        
    print(relevant_links)
    cc_news_crawl_names = relevant_links
    __logger.info('found %i files at commoncrawl.org', len(cc_news_crawl_names))
    __logger.info('found %i relevant files at commoncrawl.org', len(relevant_links))
